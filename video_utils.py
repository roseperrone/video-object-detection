...
def get_video(url, use_cache):
  ...
  if exists(mp4_filename) and use_cache:
    return mp4_filename
  system('rm -f ' + mp4_filename)
...
def get_prepared_images(url, ms_between_frames, use_cache):
  '''
  Returns:
  ...
  video_filename = get_video(url, use_cache)
